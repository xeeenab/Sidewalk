...
export const createReport = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!description) {
      return next(new AppError("Description is required", 400, "MISSING_DESCRIPTION"));
    }

    const contentHash = crypto.createHash("sha256").update(description, "utf8").digest("hex");
...
export const updateReportStatus = async (
  ...
  try {
    const {
      ...
      status,
      evidence,
    } = req.body as { originalTxHash?: string; status?: string; evidence?: string };
    ...
    const statusHash = crypto.createHash("sha256").update(dataToHash, "utf8").digest("hex");
...
export const verifyStatus = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const dataToHash = `${originalTxHash}:${status}:${evidence ?? ""}`;
    const expectedHash = crypto.createHash("sha256").update(dataToHash, "utf8").digest("hex");
    const result = await stellarService.verifyTransaction(statusTxHash, expectedHash);

    if (!result.valid) {
      return res.status(409).json({
        success: false,
        message: "Broken chain. The status update does not match the provided report data.",
      });
    }
